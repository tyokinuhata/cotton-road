<div class="jumbotron">
    ...
    <form method="POST" action="{{ url('/seller/info/notice/product/back') }}" class="d-inline">
        @csrf
        <input type="hidden" name="notice_id" value="{{ $notice->id }}">
        <input type="hidden" name="product_id" value="{{ $notice->product_id }}">
        <button type="submit" class="btn btn-success">返送</button>
    </form>
    <form method="POST" action="{{ url('/seller/info/notice/product/disposal') }}" class="d-inline">
        @csrf
        <input type="hidden" name="notice_id" value="{{ $notice->id }}">
        <input type="hidden" name="product_id" value="{{ $notice->product_id }}">
